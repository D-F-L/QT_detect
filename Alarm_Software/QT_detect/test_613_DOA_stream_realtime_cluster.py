...
def vector_doa_one_point(pt_buffer, vx_buffer, vy_buffer, fs, buffer_start_time,
                         target_time, target_freq, win_len=10, mode='center'):
    """单点 DOA 估计"""
    ...
    valid = (np.isfinite(est_angle) & np.isfinite(weight) &
...
class StreamLineDOAClusterProcessor:
    ...
    def _process_one_window(self, window_end_time):
        ...
        print(f"[PROCESS] window=[{window_start_time:.1f},{window_end_time:.1f}) "
              f"output=[{output_start:.1f},{output_end:.1f}) points={len(selected_points)}")
    ...
    def _process_pending_doa(self):
        ...
        for p in self.pending_doa_points:
            ...
            doa = vector_doa_one_point(
                ...
                self.buffer_start_time, target_time=t, target_freq=f,
                win_len=self.doa_win_len, mode=self.doa_mode
            )
            if doa is None:
                remain.append(p)
                continue
