...
def test_deepdenoiser_stft_fragment_tiled(model, noise_spec, device='cpu',
                                           ...
                                           tile_width=10000, tile_overlap=256,
                                           max_workers=4):
    """分块并行去噪 — 沿频率轴切分，ThreadPoolExecutor 并行

    Args:
        noise_spec: 已标准化的时频图 (time, freq)
        tile_width: 每块频率宽度（像素）
        tile_overlap: 相邻块重叠宽度（像素）
        max_workers: 并行线程数
    """
...
class RealtimeClusterAnalyzer:
    ...
    def _compute_pairwise_similarity_vectorized(self, valid_tracks):
        ...
        Returns:
            pair_results: list of {'tid1', 'tid2', 'consistent'}
        ...
        # 全局时间范围（取所有轨迹的交集范围，确保有重叠）
        all_t_mins = [d['t_min'] for d in track_data]
        all_t_maxs = [d['t_max'] for d in track_data]
        ...
        # 2. 构建每条轨迹的重采样 DOA + 有效掩码
        # 使用最大范围作为统一网格（用 NaN 标记无效区域）
        grid_start = min(all_t_mins)
        grid_end = max(all_t_maxs)
        ...
        valid_mask = ~np.isnan(doa_matrix)  # (n_tracks, n_times)

        # 3. 矩阵乘法计算所有对的重叠点数
        valid_int = valid_mask.astype(np.int8)
        overlap_counts = valid_int @ valid_int.T  # (n_tracks, n_tracks)
