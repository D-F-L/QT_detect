...
def test_deepdenoiser_stft_fragment(model, noise_spec, device='cpu', thresh=0.1, line_channel=0):
    ...
    if pad_h > ori_h:
        ...
        padded[ori_h:pad_h, :ori_w] = noise_spec[-rows_to_pad:, :]
...
def vector_doa_batch(pt_buffer, vx_buffer, vy_buffer, fs, buffer_start_time,
    ...
    for (start_idx, end_idx), freq_list in time_groups.items():
        ...
        # 归一化
        p = p / (torch.abs(p).max() + 1e-10)
        x = x / (torch.abs(x).max() + 1e-10)
        y = y / (torch.abs(y).max() + 1e-10)
...
def vector_doa_one_point(pt_buffer, vx_buffer, vy_buffer, fs, buffer_start_time,
    ...
    if debug:
        print(f"  [DOA_DEBUG] total={time_total_elapsed*1000:.2f}ms "
              f"(tensor={time_to_tensor*1000:.2f}ms norm={time_normalize*1000:.2f}ms "
...
class RealtimeClusterAnalyzer:
    ...
    def analyze(self, current_time):
        ...
        # 4. 两两相似度计算
        pair_results = []
        for tid1, tid2 in combinations(valid_tracks.keys(), 2):
            tr1 = valid_tracks[tid1]
            tr2 = valid_tracks[tid2]

            similarity = self._compute_doa_similarity(tr1, tr2)
            if similarity['overlap_points'] >= self.min_overlap_points:
                pair_results.append({
                    'tid1': tid1,
                    'tid2': tid2,
                    'consistent': similarity['consistent']
                })
...
class StreamLineDOAClusterProcessor:
    ...
    def push(self, pt_chunk, vx_chunk, vy_chunk):
        ...
        if len(all_results) > 0 and result_dict['noisy_spec'] is not None and last_window_end_time is not None:
            ...
            result_dict['time_azimuth'] = self._build_time_azimuth_plot(all_results)
    ...
    def _build_time_azimuth_plot(self, doa_results, azimuth_bins=360):
...
if __name__ == "__main__":
    ...
    output_txt = f"stream_output/{pid}_realtime_cluster_result.txt"

    # StreamLineDOAClusterProcessor 参数
    process_window = 240.0  # 最大窗口600s
    process_hop = 20.0
    raw_buffer_seconds = 620.0  # 需要比process_window大一些
    stft_win_seconds = 20.0
    stft_hop_seconds = 1.0
    doa_delay = 5.0
    doa_win_len = 10.0
        ...
        if len(results['doa_results']) > 0:
            ...
            with open(output_txt, 'a', encoding='utf-8') as f:
                for r in results['doa_results']:
                    ...
                    f.write(line)
            # noisy = results['noisy_spec']  # shape: (freq_bins, time_bins)
            # denoise = results['denoise_spec']  # shape: (freq_bins, time_bins)
            # doa_mat = results['doa_matrix']  # shape: (freq_bins, time_bins), 值为DOA角度
            # time_az = results['time_azimuth']
            # print('noisy.shape', noisy.shape)
            # print('denoise.shape', denoise.shape)
            # print('doa_mat.shape', doa_mat.shape)
            # print('time_az.shape', time_az.shape)
