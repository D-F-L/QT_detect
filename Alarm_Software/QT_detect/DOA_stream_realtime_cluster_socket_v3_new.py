...
def _to_numpy(data):
    """Convert Torch/NumPy data to a CPU NumPy array for legacy image/tracing code."""
    if isinstance(data, torch.Tensor):
        return data.detach().cpu().numpy()
    return np.asarray(data)


def wave_to_spec_torch(wave, frequency_resolution, fs, f_lower_bound, f_higher_bound,
                       device='cpu'):
    """Torch STFT path. Returns a (time, frequency) tensor on the target device."""
    f_lower_bound = int(f_lower_bound)
    f_higher_bound = int(f_higher_bound)
    assert len(wave.shape) == 2

    x = torch.as_tensor(wave, dtype=torch.float32, device=device).reshape(-1)
    max_val = torch.max(x)
    if torch.isfinite(max_val) and torch.abs(max_val) > 1e-12:
        x = x / max_val

    n_fft = int(fs * frequency_resolution)
    hop_length = int(fs)
    window = torch.hamming_window(n_fft, periodic=True, dtype=torch.float32, device=device)
    ...
    stft_magnitude = 20.0 * torch.log10(torch.abs(stft_result).clamp_min(1e-12))
    freqs = torch.linspace(0, fs / 2, n_fft // 2 + 1, device=device)
    freq_indices = (freqs >= f_lower_bound) & (freqs <= f_higher_bound)
    spec = stft_magnitude[:, freq_indices].to(torch.float32)

    spec_max = torch.max(spec)
    if torch.isfinite(spec_max) and torch.abs(spec_max) > 1e-12:
        spec = spec / spec_max
    return spec
...
def test_deepdenoiser_stft_fragment(model, noise_spec, device='cpu', thresh=0.1, line_channel=0):
    """对单张时频图做深度模型去噪"""
    if isinstance(noise_spec, torch.Tensor):
        noise_tensor = noise_spec.to(device=device, dtype=torch.float32)
    else:
        noise_tensor = torch.as_tensor(noise_spec, dtype=torch.float32, device=device)

    ori_h, ori_w = noise_tensor.shape
    ...
    # 创建pad后的数组
    padded = torch.zeros((pad_h, pad_w), dtype=torch.float32, device=device)

    # 先复制原始数据
    padded[:ori_h, :ori_w] = noise_tensor

    # 高度方向：用最后rows_to_pad行填充
    if pad_h > ori_h:
        rows_to_pad = pad_h - ori_h
        rows_to_pad = min(rows_to_pad, ori_h)
        padded[ori_h:ori_h + rows_to_pad, :ori_w] = torch.flip(noise_tensor[-rows_to_pad:, :], dims=[0])

    # 宽度方向：用最后几列填充
    if pad_w > ori_w:
        cols_to_pad = pad_w - ori_w
        cols_to_pad = min(cols_to_pad, ori_w)
        padded[:ori_h, ori_w:ori_w + cols_to_pad] = noise_tensor[:, -cols_to_pad:]
        # 右下角也填充
        if pad_h > ori_h:
            rows_to_pad = min(pad_h - ori_h, ori_h)
            padded[ori_h:ori_h + rows_to_pad, ori_w:ori_w + cols_to_pad] = noise_tensor[-rows_to_pad:, -cols_to_pad:]

    x = padded.unsqueeze(0).repeat(3, 1, 1).unsqueeze(0)

    with torch.inference_mode():
        res = model(x, flag='test')
        res = torch.softmax(res, dim=1)
        conf = res[0, line_channel]

        # 只取原始尺寸的部分，去掉所有pad
        conf = conf[:ori_h, :ori_w]

        conf_soft = torch.where(conf < thresh, torch.zeros_like(conf), conf)
        foreground = conf_soft * noise_tensor

        conf_bin = (conf_soft >= thresh).to(torch.int32)
...
def test_deepdenoiser_stft_fragment_tiled(model, noise_spec, device='cpu',
    ...
    if ori_w <= tile_width + tile_overlap:
        return test_deepdenoiser_stft_fragment(model, noise_tensor, device=device,
                                               thresh=thresh, line_channel=line_channel)

    step = max(1, tile_width - tile_overlap)
    tile_starts = list(range(0, max(1, ori_w - tile_width + 1), step))
    tile_starts.append(ori_w - tile_width)
    tile_starts = sorted(set(s for s in tile_starts if s >= 0 and s + tile_width <= ori_w))

    results = [
        test_deepdenoiser_stft_fragment(
            model, noise_tensor[:, s:s + tile_width], device=device,
            thresh=thresh, line_channel=line_channel
        )
        for s in tile_starts
    ]

    foreground_full = torch.zeros_like(noise_tensor, dtype=torch.float32, device=device)
    conf_full = torch.zeros_like(noise_tensor, dtype=torch.int32, device=device)
    ...
    for s, (fg, cf) in zip(tile_starts, results):
        ...
        if s > 0:
            ...
            w = torch.linspace(0, 1, overlap_end - overlap_start, dtype=torch.float32, device=device)
...
def extract_multi_lines_optimized(img, out_prefix='multi', prominence=2.5,
                                  min_length=5, delta_f=3, fre_range=40):
    """Online line extraction with vectorized peak detection and local track linking."""
    img = _to_numpy(img).astype(np.float32, copy=False)
    if img.ndim != 2:
        raise ValueError("extract_multi_lines_optimized expects a 2-D spectrogram")

    T, F = img.shape
    if T == 0 or F == 0:
        empty = np.ones((T, F, 3), dtype=np.float32)
        return empty, empty.copy(), []

    img_mean = float(np.mean(img))
    if not np.isfinite(img_mean):
        img_mean = 0.0

    mask = img > (prominence * img_mean)
    local_max = img == maximum_filter(img, size=(1, 6), mode='nearest')
    peak_mask = mask & local_max

    active = []
    finished = []
    half_range = max(1, int(fre_range) // 2) if isinstance(fre_range, int) and fre_range > 0 else int(delta_f)
    max_jump = max(1, int(delta_f))

    for t in range(T):
        peak_freqs = np.flatnonzero(peak_mask[t])
        if peak_freqs.size > 1:
        ...
        for tr in active:
            last_f = tr['last_f']
            lo = max(0, last_f - half_range)
            hi = min(F - 1, last_f + half_range)

            left = np.searchsorted(peak_freqs, lo, side='left')
            right = np.searchsorted(peak_freqs, hi, side='right')
            cand_idx = np.arange(left, right)
            if cand_idx.size:
                cand_idx = cand_idx[~used[cand_idx]]

            if cand_idx.size:
                diffs = np.abs(peak_freqs[cand_idx] - last_f)
                cand_idx = cand_idx[diffs <= max_jump]

            if cand_idx.size:
                amps = img[t, peak_freqs[cand_idx]]
                score = np.abs(peak_freqs[cand_idx] - last_f) - 1e-3 * amps
                best_local = int(cand_idx[np.argmin(score)])
                f = int(peak_freqs[best_local])
                used[best_local] = True
                tr['points'].append((t, f))
                tr['last_f'] = f
                tr['last_t'] = t
                next_active.append(tr)
            elif len(tr['points']) >= min_length:
                finished.append(np.asarray(tr['points'], dtype=np.int32))

        for idx, f in enumerate(peak_freqs):
            if used[idx]:
                continue
            f = int(f)
            next_active.append({'points': [(t, f)], 'last_f': f, 'last_t': t})

        active = next_active

    for tr in active:
        if len(tr['points']) >= min_length:
            finished.append(np.asarray(tr['points'], dtype=np.int32))

    peaks_img_vis = np.ones((T, F, 3), dtype=np.float32)
    peak_t, peak_f = np.where(peak_mask)
    for offset in range(-2, 3):
        ff = np.clip(peak_f + offset, 0, F - 1)
        peaks_img_vis[peak_t, ff, :] = [1.0, 0.0, 0.0]

    trace_img_vis = np.ones((T, F, 3), dtype=np.float32)
    for tr in finished:
        if len(tr) == 0:
            continue
        tt = tr[:, 0].astype(int)
        ff = tr[:, 1].astype(int)
        trace_img_vis[tt, ff, :] = [0.0, 0.0, 1.0]

    print(f'[MultiFast] Done! extracted {len(finished)} line tracks')
    return trace_img_vis, peaks_img_vis, finished


class DOAFFTCache:
    """Caches 3-channel FFTs for repeated DOA windows."""

    def __init__(self, fs, win_len, mode='center', device='cpu', max_items=512, nbins=720):
        self.fs = fs
        self.win_len = float(win_len)
        self.mode = mode
        self.device = device
        self.max_items = int(max_items)
        self.nfft = int(round(self.win_len * self.fs))
        self.half_nfft = self.nfft // 2
        self.nbins = int(nbins)
        self.cache = {}
        self.order = []
        self.window_cache = {}
        self.theta_centers = (torch.arange(self.nbins, dtype=torch.float32, device=device) + 0.5) * (360.0 / self.nbins)

    def configure(self, fs, win_len, mode, device):
        nfft = int(round(float(win_len) * fs))
        if fs != self.fs or nfft != self.nfft or mode != self.mode or device != self.device:
            self.__init__(fs, win_len, mode=mode, device=device, max_items=self.max_items, nbins=self.nbins)

    def trim_before(self, abs_start_sample):
        remove = [key for key in self.cache if key[0] < abs_start_sample]
        for key in remove:
            self.cache.pop(key, None)
        if remove:
            self.order = [key for key in self.order if key in self.cache]

    def _window(self, sig_len):
        win = self.window_cache.get(sig_len)
        if win is None:
            win = torch.hann_window(sig_len, periodic=True, dtype=torch.float32, device=self.device)
            self.window_cache[sig_len] = win
        return win

    def get_ffts(self, pt_buffer, vx_buffer, vy_buffer, buffer_start_time, start_idx, end_idx):
        abs_start = int(round(buffer_start_time * self.fs)) + int(start_idx)
        sig_len = int(end_idx - start_idx)
        key = (abs_start, sig_len)
        cached = self.cache.get(key)
        if cached is not None:
            return cached

        p_np = np.asarray(pt_buffer[start_idx:end_idx], dtype=np.float32)
        x_np = np.asarray(vx_buffer[start_idx:end_idx], dtype=np.float32)
        y_np = np.asarray(vy_buffer[start_idx:end_idx], dtype=np.float32)

        p = torch.as_tensor(p_np, dtype=torch.float32, device=self.device)
        ...
        p = p / torch.clamp(torch.max(torch.abs(p)), min=1e-10)
        x = x / torch.clamp(torch.max(torch.abs(x)), min=1e-10)
        y = y / torch.clamp(torch.max(torch.abs(y)), min=1e-10)

        win = self._window(sig_len)
        signals = torch.stack([p * win, x * win, y * win])
        ffts = torch.fft.rfft(signals, n=self.nfft, dim=1) / max(sig_len, 1)
...
def vector_doa_batch(pt_buffer, vx_buffer, vy_buffer, fs, buffer_start_time,
    ...
    pt_buffer = np.asarray(pt_buffer, dtype=np.float32).flatten()
    vx_buffer = np.asarray(vx_buffer, dtype=np.float32).flatten()
    vy_buffer = np.asarray(vy_buffer, dtype=np.float32).flatten()

    current_time = buffer_start_time + len(pt_buffer) / fs
    nfft = int(win_len * fs)
    half_nfft = nfft // 2
    if fft_cache is None:
        fft_cache = DOAFFTCache(fs, win_len, mode=mode, device=device)
    else:
        fft_cache.configure(fs, win_len, mode, device)
    ...
    for (start_idx, end_idx), freq_list in time_groups.items():
        ffts = fft_cache.get_ffts(
            pt_buffer, vx_buffer, vy_buffer,
            buffer_start_time, start_idx, end_idx
        )
        ...
        for idx, target_freq in freq_list:
            ...
            est_angle = torch.remainder(torch.atan2(Pvy2, Pvx2) * (180.0 / np.pi) + 180.0, 360.0)
            ...
            bin_idx = torch.clamp((est_angle * (fft_cache.nbins / 360.0)).long(), 0, fft_cache.nbins - 1)

            valid = torch.isfinite(est_angle) & torch.isfinite(weight)

            if not torch.any(valid):
                results[idx] = -5
                continue

            count_weight = torch.zeros(fft_cache.nbins, dtype=torch.float32, device=device)
            count_weight.scatter_add_(0, bin_idx[valid], weight[valid])

            max_idx = torch.argmax(count_weight)
            doa = fft_cache.theta_centers[max_idx].item()
            results[idx] = doa
...
class RealtimeClusterAnalyzer:
    """Realtime trajectory clustering with bounded storage and candidate pruning."""
    ...
    def analyze(self, current_time):
        ...
        self._prune_old_points(cutoff_time)
        active_tracks = filter_tracks_by_time_window(self.all_tracks, current_time, self.analysis_window)
        ...
        for tid, tr in active_tracks.items():
            prepared = self._prepare_track(tr, cutoff_time)
            if prepared is None:
                continue
            doa_slope = self._fit_doa_slope(prepared['time'], prepared['doa_smooth'])
            doa_disp = self._circular_dispersion(prepared['doa_smooth'])
            if abs(doa_slope) < self.min_abs_doa_slope or doa_disp > self.max_doa_disp:
                continue

            valid_tracks[tid] = prepared
            features[tid] = {
                't_min': prepared['time'][0],
                't_max': prepared['time'][-1],
                'dt': prepared['dt'],
                'mean_doa': circular_mean_deg(prepared['doa_smooth']),
                'median_freq': float(np.median(prepared['freq'])),
                'slope': doa_slope,
                'disp': doa_disp
            }
        ...
        candidate_pairs = self._candidate_pairs(features)
        pair_results, consistent_count = self._compute_pairwise_similarity_pruned(valid_tracks, candidate_pairs)
    ...
    def _prune_old_points(self, cutoff_time):
        remove_tids = []
        for tid, tr in self.all_tracks.items():
            if tr['t_end'] < cutoff_time:
                remove_tids.append(tid)
                continue
            times = np.asarray(tr['time'], dtype=np.float64)
            keep = times >= cutoff_time
            if keep.all():
                continue
            tr['time'] = list(times[keep])
            tr['freq'] = list(np.asarray(tr['freq'], dtype=np.float64)[keep])
            tr['doa'] = list(np.asarray(tr['doa'], dtype=np.float64)[keep])
            if tr['time']:
                tr['t_start'] = tr['time'][0]
                tr['t_end'] = tr['time'][-1]
            else:
                remove_tids.append(tid)
        for tid in remove_tids:
            self.all_tracks.pop(tid, None)

    def _prepare_track(self, tr, cutoff_time):
        if len(tr['time']) < 2:
            return None
        t = np.asarray(tr['time'], dtype=np.float64)
        f = np.asarray(tr['freq'], dtype=np.float32)
        doa = np.asarray(tr['doa'], dtype=np.float32)
        mask = (t >= cutoff_time) & np.isfinite(t) & np.isfinite(f) & np.isfinite(doa) & (doa >= 0)
        if mask.sum() < 2:
            return None
        t = t[mask]
        f = f[mask]
    ...
    def _candidate_pairs(self, features):
        tids = list(features.keys())
        pairs = []
        coarse_doa_thr = max(self.doa_mean_thr * 2.0, 60.0)
        coarse_slope_thr = max(self.min_abs_doa_slope * 20.0, 1.0)
        ...
        for i in range(len(tids)):
            ...
            for j in range(i + 1, len(tids)):
                tid2 = tids[j]
                f2 = features[tid2]
                overlap_start = max(f1['t_min'], f2['t_min'])
                overlap_end = min(f1['t_max'], f2['t_max'])
                if overlap_end <= overlap_start:
                    continue
                dt = min(f1['dt'], f2['dt'])
                if not np.isfinite(dt) or dt <= 0:
                    continue
                if int((overlap_end - overlap_start) / dt) < self.min_overlap_points:
                    continue
                if angular_diff_deg(f1['mean_doa'], f2['mean_doa']) > coarse_doa_thr:
                    continue
                if abs(f1['slope'] - f2['slope']) > coarse_slope_thr:
                    continue
                pairs.append((tid1, tid2))
        return pairs
    ...
    def _apply_k_core(self, graph, k):
        ...
        while changed:
            changed = False
            to_remove = [node for node in list(graph.keys()) if len(graph[node]) < k]
            if to_remove:
                changed = True
    ...
    def _compute_doa_similarity(self, tr1, tr2):
        ...
        if t_end <= t_start or len(t1) < 2 or len(t2) < 2:
            return {'overlap_points': 0, 'consistent': False}
        dt = min(tr1.get('dt', np.median(np.diff(t1))), tr2.get('dt', np.median(np.diff(t2))))
    ...
    def _compute_pairwise_similarity_pruned(self, valid_tracks, candidate_pairs):
        pair_results = []
        consistent_count = 0
        for tid1, tid2 in candidate_pairs:
            sim = self._compute_doa_similarity(valid_tracks[tid1], valid_tracks[tid2])
            consistent = sim.get('consistent', False)
            if consistent:
                consistent_count += 1
            pair_results.append({
                'tid1': tid1,
    ...
    def _compute_pairwise_similarity_vectorized(self, valid_tracks):
        features = {}
        for tid, tr in valid_tracks.items():
            features[tid] = {
                't_min': tr['time'][0],
                't_max': tr['time'][-1],
                'dt': tr.get('dt', np.median(np.diff(tr['time']))),
                'mean_doa': circular_mean_deg(tr['doa_smooth']),
                'median_freq': float(np.median(tr.get('freq', [0]))),
                'slope': self._fit_doa_slope(tr['time'], tr['doa_smooth']),
                'disp': self._circular_dispersion(tr['doa_smooth'])
            }
        return self._compute_pairwise_similarity_pruned(valid_tracks, self._candidate_pairs(features))
...
class StreamLineDOAClusterProcessor:
    ...
    def __init__(
        ...
        self.doa_fft_cache = DOAFFTCache(
            self.fs, self.doa_win_len, mode=self.doa_mode, device=self.device
        )

        if self.timing_csv is not None:
            timing_dir = os.path.dirname(self.timing_csv)
            if timing_dir != "":
                os.makedirs(timing_dir, exist_ok=True)
            self._timing_file = open(self.timing_csv, 'a', newline='', encoding='utf-8')
            fieldnames = [
                'wall_time',
                'stream_time',
                'stage',
            ...
            self._timing_writer = csv.DictWriter(self._timing_file, fieldnames=fieldnames)
            if self._timing_file.tell() == 0:
                self._timing_writer.writeheader()

    def _append_timing_row(self, row):
        ...
        self._timing_writer.writerow({
            ...
            'valid_tracks': row.get('valid_tracks', ''),
            'clusters': row.get('clusters', ''),
            'pairs': row.get('pairs', ''),
            'consistent_pairs': row.get('consistent_pairs', ''),
            'total_s': row.get('total_s', ''),
            'stft_s': row.get('stft_s', ''),
            'denoise_s': row.get('denoise_s', ''),
            'extract_s': row.get('extract_s', ''),
            'associate_s': row.get('associate_s', ''),
            'doa_s': row.get('doa_s', ''),
            'cluster_s': row.get('cluster_s', ''),
            'avg_ms_per_point': row.get('avg_ms_per_point', ''),
        })
    ...
    def push(self, pt_chunk, vx_chunk, vy_chunk):
        ...
        if len(doa_results) > 0:
            ...
            if self.timing_csv is not None:
                self._append_timing_row({
                    'wall_time': time.time(),
                    'stream_time': self.current_time,
                    'stage': 'cluster',
                    'points': len(doa_results),
                    'tracks': analysis_result['track_count'],
                    'valid_tracks': analysis_result.get('valid_track_count', 0),
                    'clusters': analysis_result['cluster_count'],
                    'pairs': analysis_result['pair_count'],
                    'consistent_pairs': analysis_result['consistent_pair_count'],
                    'total_s': elapsed,
                    'doa_s': time_doa,
                    'cluster_s': time_cluster,
                })
        ...
        if len(window_specs) > 0:
            ...
            if noisy_full is not None:
                ...
                result_dict['noisy_spec'] = _to_numpy(noisy_full[-rows_to_keep:, :])
                result_dict['denoise_spec'] = _to_numpy(denoise_full[-rows_to_keep:, :])
    ...
    def _process_one_window(self, window_end_time):
        ...
        empty_result = {'points': [], 'noisy_spec': None, 'denoise_spec': None}
        ...
        # 3. 线谱提取
        t3 = time.time()
        trace_img_vis, peaks_img, trajs = extract_multi_lines_optimized(
        ...
        if self.timing_csv is not None:
            self._append_timing_row({
                'wall_time': time.time(),
                'stream_time': self.current_time,
                'stage': 'process_window',
                'window_start': window_start_time,
                'window_end': window_end_time,
                'points': len(selected_points),
                'total_s': elapsed,
                'stft_s': time_stft,
                'denoise_s': time_denoise,
                'extract_s': time_extract,
                'associate_s': time_associate,
            })
    ...
    def _wave_to_spec_window(self, pt_win):
        ...
        current_std = float(torch.std(spec).item())
    ...
    def _process_pending_doa(self):
        ...
        if len(points_to_process) > 0:
            abs_start_sample = int(round(self.buffer_start_time * self.fs))
            self.doa_fft_cache.trim_before(abs_start_sample)
        ...
        if len(points_to_process) > 0:
            ...
            if self.timing_csv is not None:
                self._append_timing_row({
                    ...
                    'stream_time': self.current_time,
                    'stage': 'doa',
                    'points': len(points_to_process),
                    'total_s': elapsed,
                    'doa_s': elapsed,
                    'avg_ms_per_point': avg_time * 1000.0,
                })
...
if __name__ == "__main__":
    ...
    timing_csv = output_txt.replace(".txt", "_timing.csv")
    # timing_csv = None
    with open(timing_csv, 'w', newline='', encoding='utf-8'):
        pass
    ...
    try:
        ...
        print(f"[INFO] timing saved to: {timing_csv}")
