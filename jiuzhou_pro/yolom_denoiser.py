...
class YoloDenoiser(nn.Module):
    def __init__(self, out_channel=2):
        ...
        yolomodel = YOLO(model="yolo26m-denoise.yaml")
        yolomodel.load("yolo26_weights/yolo26m.pt")
        self.yolomodel = yolomodel.model

        self.upSampleLayer1 = UpsampleLayer(in_planes=128, out_planse=32, stride=(2, 2))
...
#     inputs = torch.randn([1, 3, 288, 10016]).cuda()
