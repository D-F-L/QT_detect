...
def vector_doa(ptData, vxData, vyData, fs, lineRecords, device='cpu'):
    ...
    groups = {uid: np.where(inverse_indices == i)[0] for i, uid in enumerate(unique_ids)}

    # 针对每个目标ID进行DOA估计
    for id_val in unique_ids:
        row_indices = groups[id_val]
        ...
        # 批量计算时间索引
        start_indices = np.maximum(np.floor(tar_time[:, 0] * fs).astype(int), 0)
        end_indices = np.minimum(np.floor(tar_time[:, 1] * fs).astype(int), len(ptData))

        # 过滤无效范围
        valid_mask = (end_indices > start_indices) & ((end_indices - start_indices) >= win_Len * fs)

        if not np.any(valid_mask):
            continue

        # 找到最大信号长度用于padding
        sig_lens = end_indices - start_indices
        max_sig_len = int(np.max(sig_lens[valid_mask]))
        Nfft_local = min(Nfft, max_sig_len)

        # 提取所有有效信号段并padding
        valid_indices = np.where(valid_mask)[0]
        num_valid = len(valid_indices)

        # 预分配padded信号矩阵 [num_valid, 3, max_sig_len]
        padded_signals = torch.zeros((num_valid, 3, max_sig_len), dtype=torch.float32, device=device)

        for i, n in enumerate(valid_indices):
            start_idx = start_indices[n]
            end_idx = end_indices[n]
            ...
            # 加窗并填充
            padded_signals[i, 0, :sig_len] = p * win
            padded_signals[i, 1, :sig_len] = x * win
            padded_signals[i, 2, :sig_len] = y * win

        # 批量FFT [num_valid, 3, Nfft_local]
        sig_lens_tensor = torch.from_numpy(sig_lens[valid_indices]).float().to(device).view(-1, 1, 1)
        ffts = torch.fft.fft(padded_signals, n=Nfft_local, dim=2) / sig_lens_tensor
        ...
        for i, n in enumerate(valid_indices):
            ...
            # 提取频段
            p_seg = ffts[i, 0, fLn - 1:fHn]
            x_seg = ffts[i, 1, fLn - 1:fHn]
            y_seg = ffts[i, 2, fLn - 1:fHn]
            ...
            if torch.all(Af == 0):
                ...
                if max_Af == 0 or not torch.isfinite(max_Af):
                    ...
                    AddN = (Af / max_Af).float()
                    bin_idx = torch.searchsorted(theta_torch, est_angle, right=False)
                    bin_idx = torch.clamp(bin_idx, 0, nbins - 1).long()

                    valid = torch.isfinite(est_angle) & torch.isfinite(AddN) & (bin_idx >= 0) & (bin_idx < nbins)
